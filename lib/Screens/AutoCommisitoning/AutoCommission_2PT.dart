...
class AutoCommission2PTScreen extends StatefulWidget {
  ...
  @override
  State<AutoCommission2PTScreen> createState() =>
      _AutoCommission2PTScreenState();
}

class _AutoCommission2PTScreenState extends State<AutoCommission2PTScreen> {
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        if (i != -1 && i + 7 <= res.length) {
          ...
          double? parsedFirmwareVersion = double.tryParse(substring);

          if (parsedFirmwareVersion != null) {
            setState(() {
              firmwareversion = parsedFirmwareVersion;

              if (firmwareversion! >= 0.0 && firmwareversion! <= 6.2) {
                isCurrectFirmware = true; // OK
              } else {
                isCurrectFirmware = false; // Faulty
              }
            });
        ...
        if (attempt >= _maxRetries) {
          setState(() {
            isCurrectFirmware = false; // Mark as faulty if all attempts fail
      ...
      return pdfWidgets.Container(
        child: pdfWidgets.Column(
            ...
            children: [
              ...
              pdfWidgets.Row(
                children: [
                  pdfWidgets.Text('Application Version:  ',
                      style: pdfWidgets.TextStyle(
                          fontWeight: pdfWidgets.FontWeight.bold)),
                  pdfWidgets.Text("v2.8.3")
                ],
