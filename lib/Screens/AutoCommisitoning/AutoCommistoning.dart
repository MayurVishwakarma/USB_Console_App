// ignore_for_file: must_be_immutable, unused_field, avoid_print, non_constant_identifier_names, prefer_final_fields, unnecessary_null_comparison, use_build_context_synchronously
...
*/
    ...
    if (_serialData.join().contains('INTG')) {
      if (_serialData.join().contains('BOCOM1')) {
        ...
        hexIntgValue = reverseString(
          reverseString(_response.join()).substring(0, 170),
        );
      }
    } else {
  ...
  // Removed unused _showSuccessToast method
  ...
  Future<void> setResteDatetime() async {
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        String data = "${('dts 2020 01 01 01 01 01').toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));
  ...
  Future<void> checkOutletPTTest(
    ...
    int attempt = 0;
    bool isSuccessful = false;
    bool hasShownBoosterPumpDialog = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        var aino = noOfPfcmds + 2;
        if (!hasShownBoosterPumpDialog && aiNumber == aino && pressure == 1.0) {
          hasShownBoosterPumpDialog = true;
          await closeAllSolenoids();
                    ...
                    onPressed: () async {
                      ...
                      if (pressure == 3.0) {
                        ...
                        for (int i = 0; i < noOfPfcmds; i += 1) {
                          ...
                          double roundedPtValue = double.parse(
                            ptValue!.toStringAsFixed(1),
                          );
                          print(
  ...
  Future<void> setSovAutoMode(int sovNumber) async {
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        String data = "${'SMODE $sovNumber 1 0 0'.toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));

        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          String res = _serialData.join('');
          if (res.toUpperCase().contains('SMODE $sovNumber 1 0 0')) {
