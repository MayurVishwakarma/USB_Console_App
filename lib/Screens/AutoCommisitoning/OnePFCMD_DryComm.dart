...
class OnePFCMDScreen extends StatefulWidget {
  NodeDetailsModel? data;
  String? projectName;
  OnePFCMDScreen(NodeDetailsModel nodedata, String project, {super.key}) {
    data = nodedata;
    projectName = project;
  }

  @override
  State<OnePFCMDScreen> createState() => _OnePFCMDScreenState();
}

class _OnePFCMDScreenState extends State<OnePFCMDScreen> {
...
*/
  ...
  // void loadCheckList() {
  //   final jsonString = CheckList().checkList;
  ...
  void initState() {
    ...
    _getPorts();
    // loadCheckList();
    // Calculate `noOfPfcmds` and initialize lists
    var subChak = widget.data?.subChakQty ?? 0;
    noOfPfcmds =
        (subChak / 2).round(); // Rounded if `noOfPfcmds` needs to be an int
    initializeLists();
  }
    ...
    while (attempt < maxRetries && !isSuccessful) {
      try {
        ...
        if (difference > 0.5) {
          ...
          showCheckPTValuesPopup(context, pressure, updatedFilterInlet);
          isSuccessful = true; // Mark as successful
        }
      } catch (ex) {
        attempt++;
...
*/
  Future<void> performPressure3Checks(BuildContext context) async {
    ...
    await checkOutletPTTest(
      ...
      (value) => outletPT_Status_3bar_open?[1] = value,
