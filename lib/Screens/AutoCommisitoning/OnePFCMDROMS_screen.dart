...
class OnePFCMDROMSScreen extends StatefulWidget {
  NodeDetailsModel? data;
  String? projectName;
  OnePFCMDROMSScreen(NodeDetailsModel nodedata, String project, {super.key}) {
    data = nodedata;
    projectName = project;
  }

  @override
  State<OnePFCMDROMSScreen> createState() => _OnePFCMDROMSScreenState();
}

class _OnePFCMDROMSScreenState extends State<OnePFCMDROMSScreen> {
  ...
  List<String> outletPT_Status_3bar_close = [];
  List<double> outletPT_Values_3bar_close = [];

  List<String>? outletPT_Status_3bar_open = [];
  List<double>? outletPT_Values_3bar_open = [];

  List<String>? outletPT_Status_1bar_open = [];
  List<double>? outletPT_Values_1bar_open = [];
...
*/
  ...
  void initState() {
    super.initState();
    // Listen for USB events and update ports if an event occurs
    ...
    // Initialize ports and load checklist
    _getPorts();
    var subChak = widget.data?.subChakQty ?? 0;
    noOfPfcmds = subChak; // Rounded if `noOfPfcmds` needs to be an int
    initializeLists();
  }
  ...
  void initializeLists() {
    ...
    outletPT_Status_3bar_close = List.filled(noOfPfcmds, '');
    outletPT_Values_3bar_close = List.filled(noOfPfcmds, 0.0);

    outletPT_Status_3bar = List.filled(noOfPfcmds, '');
    outletPT_Values_3bar = List.filled(noOfPfcmds, 0.0);

    outletPT_Status_3bar_open = List.filled(noOfPfcmds, '');
    outletPT_Values_3bar_open = List.filled(noOfPfcmds, 0.0);

    outletPT_Status_1bar_open = List.filled(noOfPfcmds, '');
    outletPT_Values_1bar_open = List.filled(noOfPfcmds, 0.0);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('${widget.data?.chakNo}')),
      body: getBodyWidget(),
    );
  }
            ...
            child: Container(
              child: Column(
                 ...
                 */
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.15,
                                      child: Text(
                                        '${outletPT_Values_1bar_open?[index]} bar',
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.15,
                                      child: Text(
                                        "${outletPT_Status_1bar_open?[index]}",
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                  ],
                  ...
                  //PT Valve Check at 3 bar after Open PFCMD solenoid
                  Container(
                    margin: EdgeInsets.all(5.5),
                    decoration: BoxDecoration(
                      boxShadow: [
                              ...
                              return Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    //       0.15,
                                    //   child: Text(
                                    //     '${outletPT_Values_3bar_close?[index] ?? ''} bar',
                                    //     textAlign: TextAlign.center,
                                    //   ),
                                    // ),
                                    SizedBox(
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.15,
                                      child: Text(
                                        '${outletPT_Values_3bar_open?[1] ?? ''} bar',
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                    SizedBox(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.15,
                                      child: Text(
                                        "${outletPT_Status_3bar_close?[index]}",
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                  ],
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.15,
                                      child: Text(
                                        '${outletPT_Values_3bar_open?[index] ?? ''} bar',
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                    SizedBox(
...
*/
                    ...
                    onPressed: () async {
                      ...
                      for (int i = 0; i < 1; i += 1) {
                        ...
                        await Future.delayed(
                          Duration(seconds: 2),
                        ).whenComplete(() => setSovFlowControl());
                        await Future.delayed(
                          Duration(seconds: 2),
                        ).whenComplete(() => setSovAutoMode());
                        await Future.delayed(
                          Duration(seconds: 2),
                        ).whenComplete(() => setRModeAutoMode());
                      }
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        if (i != -1 && i + 10 <= res.length) {
          ...
          if (digits.length > 1 && (digits[1] == '1' || digits[3] == '1')) {
            setState(() {
              isDoorOpen = true;
            });
          } else {
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        if (i != -1 && i + 10 <= res.length) {
          ...
          if (digits.length > 1 && (digits[1] == '0' || digits[3] == '0')) {
            setState(() {
              isDoorClose = true;
            });
            isSuccessful =
  ...
  Future<void> checkForLeakage(double pressure) async {
    ...
    double initialFilterInlet = 0.0;
    double updatedFilterInlet = 0.0;
    double lowerLimitNew = pressure - 0.4;
    double upperLimitNew = pressure + 0.2;
    ...
    while (attempt < maxRetries && !isSuccessful) {
      try {
        ...
        // Step 1: Fetch initial value
        initialFilterInlet = await _fetchFilterInletValue();
        print("Initial Filter Inlet Value: $initialFilterInlet");
        ...
        // Step 3: Fetch updated value
        updatedFilterInlet = await _fetchFilterInletValue();
        print("Updated Filter Inlet Value: $updatedFilterInlet");

        // Step 4: Compare values
        double difference = (initialFilterInlet - updatedFilterInlet).abs();
        print("Difference in Filter Inlet Values: $difference");

        if (difference > 0.5) {
          // Declare leakage found
          ...
          await showCheckPTValuesPopup(context, pressure, updatedFilterInlet);
          isSuccessful = true; // Mark as successful
        }
      } catch (ex) {
        attempt++;
        ...
        if (attempt >= maxRetries) {
          ...
          showLeakageFoundPopup(pressure); // Show failure message
        } else {
  ...
  Future<double> _fetchFilterInletValue() async {
    try {
      String command = "AI 1\r\n".toUpperCase();
      ...
      String res = _serialData.join('');
      int startIndex = res.indexOf("AI");
      int endIndex = res.indexOf(">");

      if (startIndex != -1 && endIndex != -1 && endIndex > startIndex) {
        String substring = res.substring(startIndex + 4, endIndex).trim();
        List<String> digits =
            substring.split(' ').where((s) => s.isNotEmpty).toList();
      ...
      throw Exception("Invalid or missing response format.");
    } catch (e) {
      print("Error fetching filter inlet value: $e");
      throw e;
    }
  }
                    ...
                    onPressed: () async {
                      ...
                      if (pressure == 3.0) {
                        ...
                        await checkOutletPTTest(
                          2,
                          (status) => outletPT_3bar = status,
                          (value) => filterOutlet3bar = value,
                          'Checking filter outlet PT',
                          true,
                          false,
                          3.0,
                        ...
                        for (int i = 0; i < 1; i += 1) {
                          ...
                          checkOutletPTTest(
                            ptpin,
                            (status) {
                              outletPT_Status_3bar?[i] = status;
                            },
                            (value) => outletPT_Values_3bar?[i] = value,
                            'Checking filter Outlet PT $pin',
                            false,
                            false,
                            3.0,
                          );
                        ...
                        await checkOutletPTTest(
                          2,
                          (status) => outletPT_1bar = status,
                          (value) => filterOutlet1bar = value,
                          'Checking filter outlet PT',
                          true,
                          false,
                          1.0,
                        ...
                        for (int i = 0; i < 1; i += 1) {
                          ...
                          checkOutletPTTest(
                            ptpin,
                            (status) => outletPT_Status_1bar[i] = status,
                            (value) => outletPT_Values_1bar[i] = value,
                            'Checking filter Outlet PT $pin',
                            false,
                            false,
                            1.0,
  ...
  Future<void> performPressure1Checks() async {
    // Call the common function for Inlet PT
    await checkOutletPTTest(
      1,
      (status) => inletPT_1bar_new = status,
      (value) => filterInlet1bar_new = value,
      'Checking filter inlet PT',
      true,
      false,
      1.0,
    );
    ...
    await openAllSolenoids_sov();
    await Future.delayed(Duration(seconds: 10));

    // Call the common function for Outlet PT
    await checkOutletPTTest(
      2,
      (status) {
        outletPT_1bar_new = status;
        mainSolenoidstatus = status;
      },
    ...
    /*
    // Open all solenoids using `openAllSolenoids_sov`
    await openAllSolenoids_sov();
    await Future.delayed(Duration(seconds: 10));
    */

    // Check PT after opening the solenoid
    await checkOutletPTTest(
      3,
      (status) {
        outletPT_Status_1bar_open?[0] = status;
      },
      (value) => outletPT_Values_1bar_open?[0] = value,
      'Checking Outlet PT 1 ',
      false,
      true,
      1.0,
    );
    ...
    // Perform the position sensor check
    // await performPositionSensorCheck();
  }

  bool? isTubing = false;
  // Function to show the leakage found popup
  Future<void> showTubingChangePopup(double press) {
    return showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Tubbing Change Pop-up',
                    style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold),
                  ),
                  Text(
                    "To test the solenoid please change the tubbing.",
                    style: TextStyle(fontWeight: FontWeight.w500),
                    textAlign: TextAlign.center,
                  ),
                  SizedBox(height: 60),
...
// Function to handle checks for pressure == 3.0
  Future<void> performPressure3Checks(BuildContext context) async {
    ...
    await checkOutletPTTest(2, (status) {
      outletPT_3bar_new = status;
      mainSolenoidstatus = status;
    }, (value) => filterOutlet3bar_new = value, 'Checking filter outlet PT',
        false, true, 3.0);
    //open all sov
    await openAllSolenoids_sov();
    ...
    for (int i = 0; i < noOfPfcmds; i += 1) {
      int pin = i + 1;
      int ptpin = i + 3;
      await setSolenoidClose(pin);
      await Future.delayed(Duration(seconds: 5)).whenComplete(
        () => checkOutletPTTest(ptpin, (status) {
          outletPT_Status_3bar_open?[i] = status;
          solenoid_status?[i] = status;
        }, (value) => outletPT_Values_3bar_open?[i] = value,
            'Checking filter Outlet PT $pin', false, true, 3.0),
      );
    }
    await performPositionSensorCheck();
  }
*/
  ...
  Future<void> performPressure3Checks(BuildContext context) async {
    ...
    await checkOutletPTTest(
      3,
      (status) {
        outletPT_Status_3bar_open?[0] = status;
        solenoid_status?[1] = status;
      },
      (value) => outletPT_Values_3bar_open?[0] = value,
      'Checking Outlet PT 1 ',
      false,
      true,
      3.0,
    ...
    if (isTubing!) {
      ...
      await checkOutletPTTest(
        3,
        (status) {
          outletPT_Status_3bar_open?[1] = status;
          solenoid_status?[3] = status;
        },
        (value) => outletPT_Values_3bar_open?[1] = value,
        'Checking Outlet PT 1 ',
        false,
        true,
        3.0,
  ...
  Future<void> changeSolenoidMode() async {
    for (int i = 0; i < 1; i += 1) {
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => setSovManualMode());
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => setSovOpenCloseMode());
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => openAllSolenoids());
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => setRModeManualMode());
    }
  }
  ...
  //Open All Solenoids One By One
  Future<void> openAllSolenoids_sov() async {
    for (int i = 0; i < noOfPfcmds; i += 1) {
      int pin = i + 1;
      int ptpin = i + 3;
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => setSolenoidOpen(pin));
    }
  }

  Future<void> closeAllSolenoids_sov() async {
    for (int i = 0; i < noOfPfcmds; i += 1) {
      int pin = i + 1;
      int ptpin = i + 3;
      await Future.delayed(
        Duration(seconds: 2),
      ).whenComplete(() => setSolenoidClose(pin));
    }
  }
  ...
  Future<void> setSovFlowControl() async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        _showProcessingToast(
          context,
          content: "Setting Solenoid to Flow Control mode...",
        );
        ...
        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          ...
          if (res.toUpperCase().contains('PFCMD1TYPE 2')) {
            print("Solenoid is set to Flow Control mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV");
          }
        });
        _response = [];
      } catch (ex) {
      ...
      if (!isSuccessful) {
        attempt++;
        if (attempt < _maxRetries) {
          print("Retrying to set SOV (Attempt $attempt)...");
        } else {
          print("Failed to set SOV after $attempt attempts.");
        }
      }
    }
  }
  ...
  Future<void> setSovOpenCloseMode() async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        _showProcessingToast(
          context,
          content: "Setting Solenoid to Open/Close mode...",
        );
        ...
        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          ...
          if (res.toUpperCase().contains('PFCMD1TYPE 1')) {
            print("Solenoid is set to Open/Close mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV ");
          }
      ...
      if (!isSuccessful) {
        attempt++;
        if (attempt < _maxRetries) {
          print("Retrying to set Solenoid (Attempt $attempt)...");
        } else {
          print("Failed to set Solenoid after $attempt attempts.");
        }
      }
    }
  }
  ...
  Future<void> setSovManualMode() async {
    int attempt = 0;
    bool isSuccessful = false;
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        _showProcessingToast(
          context,
          content: "Setting Solenoid to Manual mode...",
        );
        ...
        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          ...
          if (res.toUpperCase().contains('SMODE 2 1 1')) {
            print("Solenid is set to Manual mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV");
          }
  ...
  // set Solenoid in Manual mode.
  Future<void> setSovAutoMode() async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        _showProcessingToast(
          context,
          content: "Setting Solenoid to Auto mode...",
        );
        ...
        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          ...
          if (res.toUpperCase().contains('SMODE 1 0 0')) {
            print("Solenoid is set to Auto mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV");
          }
  ...
  // set Rmode in Manual mode.
  Future<void> setRModeManualMode() async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        _showProcessingToast(
          context,
          content: "Setting RMode to Manual mode...",
        );

        String data = "${'RMODE 1'.toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));

        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          String res = _serialData.join('');
          if (res.toUpperCase().contains('RMODE 1')) {
            print("RMODE is set to Manual mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV");
          }
  ...
  // set Solenoid in Manual mode.
  Future<void> setRModeAutoMode() async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        _response.clear();
        _serialData.clear();
        _showProcessingToast(context, content: "Setting RMode to Auto mode...");

        String data = "${'RMODE 0'.toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));

        await Future.delayed(Duration(seconds: 5)).whenComplete(() {
          String res = _serialData.join('');
          if (res.toUpperCase().contains('RMODE 0')) {
            print("RMODE is set to Auto mode successfully");
            isSuccessful = true;
          } else {
            print("Failed to set SOV");
          }
  ...
  Future<void> setSovOpen(int index) async {
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        _serialData.clear();
        _showProcessingToast(context, content: "Opening Solenoid");
  ...
  //Set Solenoid Open
  Future<void> setSolenoidOpen(int index) async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        ...
        String data = "${('SOV $index 1').toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));

        await Future.delayed(Duration(seconds: 10)).whenComplete(() {
          String res = _serialData.join('');
          if (res.toLowerCase().contains('sov $index 1')) {
            print("SOV $index is OPEN now");
  ...
  Future<void> setSolenoidClose(int index) async {
    int attempt = 0;
    bool isSuccessful = false;
    ...
    while (attempt < _maxRetries && !isSuccessful) {
      try {
        _serialData.clear();
        _showProcessingToast(context, content: "Closing Solenoid");

        String data = "${('SOV $index 0').toUpperCase()}\r\n";
        await _port!.write(Uint8List.fromList(data.codeUnits));

        await Future.delayed(Duration(seconds: 10)).whenComplete(() {
          String res = _serialData.join('');
          if (res.toLowerCase().contains('sov $index 0')) {
            print("SOV $index is CLOSE  now");
  ...
  void _submitForm() {
                      ...
                      pdfWidgets.TableRow(
                        children: [
                          pdfWidgets.Container(
                            height: 20,
                            alignment: pdfWidgets.Alignment.center,
                            child: pdfWidgets.Text('PFCMD Solenoid'),
                          ),
                          pdfWidgets.Container(
                            height: 20,
                            alignment: pdfWidgets.Alignment.center,
                          ...
                          pdfWidgets.Container(
                            height: 20,
                            alignment: pdfWidgets.Alignment.center,
                            child: pdfWidgets.Text(mainSolenoidstatus ?? ''),
                          ),
                        ],
                      ),
    ...
    savePDF(pdf, context, widget.projectName ?? '', widget.data?.chakNo ?? '');
  ...
  Future<void> savePDF(
    ...
    try {
      ...
      String pdfName = 'AutoDry $chakNo-$projectName.pdf';
  ...
  Future<void> saveDeviceDataToFile(
    ...
    try {
      ...
      // String fileName = 'AutoDry $chakNo-$projectName.json';
